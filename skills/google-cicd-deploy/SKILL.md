...
8.  **Verification**: Provide the user with commands to check the deployment status, as detailed in the reference document `skills/gcp-cicd-deploy/references/how_to_deploy_to_gke_with_kubectl.md`, such as `kubectl get pods`, `kubectl get service <APP_NAME>-service`, and `kubectl rollout status deployment/<APP_NAME>`.
